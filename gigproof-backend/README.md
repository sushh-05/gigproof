...
```bash
gigproof-backend$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
gigproof-backend$ sam local start-api
gigproof-backend$ curl http://localhost:3000/
```
...
```bash
gigproof-backend$ sam logs -n HelloWorldFunction --stack-name gigproof-backend --tail
```
...
```bash
gigproof-backend$ cd hello-world
...
```bash
sam delete --stack-name gigproof-backend
```
